...
impl BuildConfig<'_> {
    ...
    pub fn cmd(&self, subcommand: &str) -> std::process::Command {
        ...
        // nightly features that we use: asm_sym,named-profiles,naked_functions,cmse_nonsecure_entry,array_methods,destructuring_assignment
        // nightly features that our dependencies use: proc_macro_span,backtrace,asm,llvm_asm

        cmd.arg(r#"-Zallow-features=asm,asm_sym,named-profiles,naked_functions,cmse_nonsecure_entry,array_methods,destructuring_assignment,proc_macro_span,backtrace,llvm_asm"#);
